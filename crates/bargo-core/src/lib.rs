...
fn dispatch(cli: &Cli, cfg: &Config) -> Result<()> {
    ...
    match &cli.command {
        ...
        Commands::Cairo { command } => match command {
            ...
            cli::CairoCommands::VerifyOnchain { address } => {
                if !cfg.quiet {
